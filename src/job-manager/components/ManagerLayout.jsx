import { CalendarDays, ChevronDown, CreditCard, Files, FolderKanban, LayoutDashboard, LogOut, ReceiptPoundSterling, ReceiptText, Settings, UserRoundSearch, X } from 'lucide-react'
...
const nav = [
  ...
  { to: '/job-manager/invoices', label: 'Invoices', icon: ReceiptText },
...
export default function ManagerLayout() {
  ...
  return <div className="jm-shell">
    ...
    <nav className="jm-bottom-nav" aria-label="Mobile navigation">{visibleNav.filter((item) => ['Dashboard','Calendar','Leads','Projects','Files','Invoices','Expenses'].includes(item.label)).map(({ to, label, icon: Icon, end }) => <NavLink key={to} to={to} end={end}><Icon size={20} /><span>{label}</span>{label === 'Leads' && attentionCount(data, user) > 0 && <b className="jm-nav-badge">{attentionCount(data, user)}</b>}</NavLink>)}</nav>
    <AssistantWidget />
  </div>
}
