...
import { applySelectedFileDefaults, friendlyUploadError, PROJECT_FILE_ACCEPT, PROJECT_FILE_HELP, uploadFileKind } from '../utils/fileUploads'
...
export function PhotoGallery({ photos, users, canDelete = () => false, onDelete, onAdd, onUpload, storageEnabled }) {
  ...
  const submit = async (event) => {
    ...
    for (let index = 0; index < pending.length; index += 1) {
      ...
      catch (uploadError) { failed += 1; setQueue((current) => current.map((entry) => entry.id === item.id ? { ...entry, status: 'error', error: friendlyUploadError(uploadError, 'Upload failed. Try this photo again.') } : entry)) }
    }
