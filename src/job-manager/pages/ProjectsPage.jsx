...
export default function ProjectsPage() {
  ...
  return <>
    ...
    {filtered.length === 0 ? <EmptyState title="No projects match" text="Try clearing one or more filters." action={<button className="jm-button jm-button--secondary" onClick={() => { setSearch(''); setStatus('All'); setPostcode(''); setFrom('') }}>Clear filters</button>} /> : view === 'cards' ?
      <div className="jm-project-grid jm-project-grid--all">{filtered.map((project) => <ProjectCard key={project.id} project={project} data={data} users={users} />)}</div> :
      <div className="jm-table-wrap"><table className="jm-table"><thead><tr><th>Client / project</th><th>Status</th><th>Location</th><th>Dates</th><th>Assigned</th><th>Value</th><th /></tr></thead><tbody>{filtered.map((project) => <tr key={project.id}><td><strong>{projectClient(data, project)?.name}</strong><span>{project.title}</span></td><td><StatusBadge status={project.status} /></td><td>{project.postcode}</td><td>{formatDate(project.startDate)} – {formatDate(project.endDate)}</td><td>{projectUser(users, project)?.name}</td><td>{formatGBP(project.contractValue)}</td><td><Link to={`/job-manager/projects/${project.id}`}>Open</Link></td></tr>)}</tbody></table></div>}
  </>
}
