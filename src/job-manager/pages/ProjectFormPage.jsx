...
export default function ProjectFormPage() {
  ...
  return <>
    ...
    <form className="jm-project-form" onSubmit={handleSubmit(submit)} noValidate>
      {Object.keys(errors).length > 0 && <p className="jm-form-error-summary" role="alert">Please check the highlighted fields below.</p>}
      ...
      <details className="jm-form-optional"><summary><div><h2>Additional project information</h2><p>Value, keys, access, parking and internal notes.</p></div><span>Optional</span></summary><section className="jm-form-section"><div><h2>Site details</h2><p>Add these details now or return to them later.</p></div><div className="jm-form-grid">{field('contractValue', 'Contract value (£)', <input type="number" min="0" step="1" {...register('contractValue')} />)}{field('keyStatus', 'Key status', <input {...register('keyStatus')} />)}{field('nextAction', 'Next action', <input {...register('nextAction')} />)}<label className="jm-field jm-field--full"><span>Access instructions</span><textarea rows="3" {...register('accessNotes')} /></label><label className="jm-field jm-field--full"><span>Parking information</span><textarea rows="3" {...register('parkingNotes')} /></label><label className="jm-field jm-field--full"><span>Internal notes</span><textarea rows="4" {...register('internalNotes')} /></label></div></section></details>
