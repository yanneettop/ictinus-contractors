...
export default function ProjectDetailPage() {
  ...
  const { data, users, can, updateProjectStatus, deleteProject, addTask, toggleTask, addPayment, markPaymentPaid, addDocument, deleteDocument, addEvent, addJournalEntry, updateJournalEntry, deleteJournalEntry, addPhoto, deletePhoto } = useJobManager()
  ...
  const submitDocument = (event) => { event.preventDefault(); addDocument(id, Object.fromEntries(new FormData(event.currentTarget))); closeActionForm(event) }
  ...
  return <>
    ...
    <div className="jm-quick-actions jm-quick-actions--control" aria-label="Project actions">
      ...
      <details><summary><FilePlus2 size={18} />Add document</summary><ActionForm onSubmit={submitDocument}><label>Name<input name="name" required /></label><label>Type<select name="type"><option>Quotation</option><option>Invoice</option><option>Payment schedule</option><option>Photos</option><option>Contract</option><option>Certificate</option><option>Google Drive folder</option><option>Other</option></select></label><label>External URL<input name="url" type="url" placeholder="https://" required /></label></ActionForm></details>
    ...
    <div className="jm-detail-layout"><div className="jm-detail-main">
      ...
      <ProjectJournal key={journalComposer} entries={journal} users={users} onAdd={(values) => addJournalEntry(id, values)} onUpdate={updateJournalEntry} onDelete={(entryId) => window.confirm('Delete this journal note?') && deleteJournalEntry(entryId)} startOpen={journalComposer > 0} />
      <ProjectTasks tasks={tasks} users={users} onToggle={toggleTask} />
      <DocumentsSection documents={documents} users={users} canDelete={can('edit_projects')} onDelete={(documentId) => window.confirm('Remove this document link?') && deleteDocument(documentId)} />
      <PhotoGallery photos={photos} users={users} canDelete={can('edit_projects')} onDelete={(photoId) => window.confirm('Delete this project photo?') && deletePhoto(photoId)} onAdd={(values) => addPhoto(id, values)} />
