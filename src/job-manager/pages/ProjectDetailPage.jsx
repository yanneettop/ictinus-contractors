import { ArrowLeft, CalendarDays, CalendarPlus, Check, CheckCircle2, ClipboardPlus, CreditCard, Edit3, FilePlus2, Mail, MapPin, MessageCircle, MessageSquarePlus, MoreHorizontal, Phone, Plus, ReceiptText, Trash2, UserRound, X } from 'lucide-react'
...
export default function ProjectDetailPage() {
  ...
  const submitDocument = async (event) => { event.preventDefault(); const formData = new FormData(event.currentTarget); const file = formData.get('file'); const values = { name: String(formData.get('name')).trim(), type: formData.get('type'), url: formData.get('url') || '' }; try { if (authMode === 'supabase') { if (!file?.size) throw new Error('Choose a file to store in the account.'); await uploadDocument(id, values, file) } else { if (!values.url) throw new Error('Add an external file URL.'); await addDocument(id, values) } closeActionForm(event) } catch (uploadError) { setMessage(friendlyUploadError(uploadError)) } }
  ...
  return <>
    ...
    {message && <div className="jm-operation-notice" role="status" aria-live="polite"><span>{message}</span><button onClick={() => setMessage('')}>Dismiss</button></div>}

    <div className="jm-project-command-bar" aria-label="Project actions"><button type="button" className="jm-button jm-button--primary jm-add-update-button" aria-expanded={addingUpdate} onClick={() => setAddingUpdate((open) => !open)}>{addingUpdate ? <X size={18} /> : <Plus size={18} />}{addingUpdate ? 'Close actions' : 'Add update'}</button><div className="jm-project-contact-actions"><a href={communicationService.phoneUrl(client)}><Phone size={18} />Call</a><a href={communicationService.whatsappUrl(client, project)} target="_blank" rel="noreferrer"><MessageCircle size={18} />WhatsApp</a><a href={locationService.mapsUrl(project)} target="_blank" rel="noreferrer"><MapPin size={18} />Directions</a></div><details className="jm-more-project-actions"><summary><MoreHorizontal size={18} />More actions</summary><div><a href={communicationService.emailUrl(client, project)}><Mail size={17} />Email client</a><a href={googleCalendarService.projectUrl(project)} target="_blank" rel="noreferrer"><CalendarDays size={17} />Add to calendar</a>{can('edit_financials') && <button type="button" onClick={generateInvoice}><ReceiptText size={17} />Generate invoice</button>}{project.status !== 'Completed' && <button type="button" onClick={() => window.confirm('Mark this project as completed? It will move to the Completed projects section.') && updateProjectStatus(id, 'Completed')}><CheckCircle2 size={17} />Mark as completed</button>}</div></details></div>

    {addingUpdate && <section className="jm-add-update-panel" aria-labelledby="jm-add-update-title"><header><div><p className="jm-eyebrow">Project update</p><h2 id="jm-add-update-title">What would you like to add?</h2></div><button type="button" onClick={() => setAddingUpdate(false)} aria-label="Close add update panel"><X size={20} /></button></header><div className="jm-quick-actions jm-quick-actions--control">
      <details><summary><ClipboardPlus size={18} />Task</summary><ActionForm onSubmit={submitTask}><label>Task title<input name="title" required /></label><label>Due date<input name="dueDate" type="date" required /></label><label>Assigned to<select name="assignedTo">{users.map((teamUser) => <option key={teamUser.id} value={teamUser.id}>{teamUser.name}</option>)}</select></label><label>Priority<select name="priority"><option>Medium</option><option>High</option><option>Low</option></select></label></ActionForm></details>
      <button type="button" onClick={openJournal}><MessageSquarePlus size={18} />Site note</button>
      <details><summary><CalendarPlus size={18} />Event</summary><ActionForm onSubmit={submitEvent}><label>Event type<select name="type">{Object.keys(eventColours).map((type) => <option key={type}>{type}</option>)}</select></label><label>Start date<input name="startDate" type="date" required /></label><label>End date<input name="endDate" type="date" /></label></ActionForm></details>
      <details><summary><FilePlus2 size={18} />Document</summary><ActionForm onSubmit={submitDocument}><label>Document name<input name="name" placeholder="Filled automatically from the file" required /></label><label>Document type<select name="type"><option>Quotation</option><option>Invoice</option><option>Payment schedule</option><option>Photos</option><option>Video</option><option>Word document</option><option>Text file</option><option>Contract</option><option>Certificate</option><option>Other</option></select></label>{authMode === 'supabase' ? <label>Choose a file<input name="file" type="file" accept={PROJECT_FILE_ACCEPT} onChange={(event) => applySelectedFileDefaults(event.currentTarget)} required /><small>{PROJECT_FILE_HELP}</small></label> : <label>External file link<input name="url" type="url" placeholder="https://" required /></label>}</ActionForm></details>
      {can('edit_financials') && <details><summary><CreditCard size={18} />Payment</summary><ActionForm onSubmit={submitPayment}><label>Payment stage<input name="title" placeholder="Deposit, mid-project…" required /></label><label>Amount (£)<input name="amount" type="number" min="0" required /></label><label>Percentage<input name="percentage" type="number" min="0" max="100" /></label><label>Due date<input name="dueDate" type="date" required /></label><label>Invoice reference<input name="invoiceReference" required /></label></ActionForm></details>}
    </div></section>}
