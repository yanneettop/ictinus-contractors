...
export default function ProjectDetailPage() {
  ...
  return <>
    ...
    <div className="jm-quick-actions jm-quick-actions--control" aria-label="Project actions">
      ...
      <details><summary><FilePlus2 size={18} />Add file</summary><ActionForm onSubmit={submitDocument}><label>Name<input name="name" required /></label><label>Type<select name="type"><option>Quotation</option><option>Invoice</option><option>Payment schedule</option><option>Photos</option><option>Video</option><option>Word document</option><option>Contract</option><option>Certificate</option><option>Other</option></select></label>{authMode === 'supabase' ? <label>Upload to private storage<input name="file" type="file" accept={PROJECT_FILE_ACCEPT} onChange={(event) => applySelectedFileDefaults(event.currentTarget)} required /><small>{PROJECT_FILE_HELP}</small></label> : <label>External URL<input name="url" type="url" placeholder="https://" required /></label>}</ActionForm></details>
