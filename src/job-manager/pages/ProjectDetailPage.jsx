...
export default function ProjectDetailPage() {
  ...
  const submitDocument = async (event) => { event.preventDefault(); const formData = new FormData(event.currentTarget); const file = formData.get('file'); const values = { name: formData.get('name'), type: formData.get('type'), url: formData.get('url') || '' }; try { if (file?.size) await uploadDocument(id, values, file); else if (values.url) await addDocument(id, values); else throw new Error('Choose a file or add an external URL.'); closeActionForm(event) } catch (uploadError) { setMessage(uploadError.message) } }
  ...
  return <>
    ...
    <div className="jm-quick-actions jm-quick-actions--control" aria-label="Project actions">
      ...
      <details><summary><FilePlus2 size={18} />Add document</summary><ActionForm onSubmit={submitDocument}><label>Name<input name="name" required /></label><label>Type<select name="type"><option>Quotation</option><option>Invoice</option><option>Payment schedule</option><option>Photos</option><option>Contract</option><option>Certificate</option><option>Google Drive folder</option><option>Other</option></select></label>{authMode === 'supabase' && <label>Upload PDF or image<input name="file" type="file" accept="application/pdf,image/jpeg,image/png,image/webp" /></label>}<label>Or external URL<input name="url" type="url" placeholder="https://" /></label></ActionForm></details>
