...
export const supabaseRepository = {
  ...
  async load() {
    ...
    const data = { version: 5, users: profiles.map(maps.users), clients: clients.map(maps.clients), projects: projects.map(maps.projects), events: events.map(maps.events), tasks: tasks.map(maps.tasks), payments: payments.map(maps.payments), documents: documents.map(maps.documents), photos: photos.map(maps.photos), journalEntries: journalEntries.map(maps.journalEntries), activities: activities.map(maps.activities), leads: leads.map(maps.leads), leadCommunications: leadCommunications.map(maps.leadCommunications), leadQuotes: leadQuotes.map(maps.leadQuotes) }
  ...
  async save(data) {
    ...
    for (const key of ['clients', 'projects', 'documents', 'leadQuotes', 'leads', 'leadCommunications', 'events', 'tasks', 'payments', 'photos', 'journalEntries', 'activities']) await syncCollection(client, key, data[key] || [], previous[key] || [])
    snapshot = clone(data)
    return data
  },
