...
const maps = {
  ...
  expenses: (row) => ({ id: row.id, projectId: row.project_id, date: row.expense_date, category: row.category, supplier: row.supplier, description: row.description, amount: pounds(row.amount_pence), paymentMethod: row.payment_method, reference: row.reference, notes: row.notes, attachmentName: row.attachment_name, attachmentType: row.attachment_mime_type, storagePath: row.storage_path, url: '', createdBy: row.created_by, createdAt: row.created_at, updatedAt: row.updated_at }),
  documents: (row) => ({ id: row.id, projectId: row.project_id, leadId: row.lead_id, type: row.type, name: row.name, url: row.external_url || '', storagePath: row.storage_path, uploadedBy: row.uploaded_by, createdAt: row.created_at }),
  ...
  leadQuotes: (row) => ({ id: row.id, leadId: row.lead_id, documentId: row.document_id, reference: row.reference, amount: pounds(row.amount_pence), status: row.status, sentAt: row.sent_at, notes: row.notes, projectTitle: row.project_title, projectType: row.project_type, description: row.description, scope: row.scope || [], address: row.address, postcode: row.postcode, startDate: row.start_date, endDate: row.end_date, createdBy: row.created_by, createdAt: row.created_at, updatedAt: row.updated_at }),
}
...
const dbMaps = {
  ...
  expenses: (row) => ({ id: row.id, project_id: row.projectId || null, expense_date: row.date, category: row.category, supplier: row.supplier || '', description: row.description, amount_pence: pence(row.amount), payment_method: row.paymentMethod || 'Company card', reference: row.reference || '', notes: row.notes || '', attachment_name: row.attachmentName || null, attachment_mime_type: row.attachmentType || null, storage_path: row.storagePath || null, created_by: row.createdBy, updated_at: row.updatedAt || new Date().toISOString() }),
  documents: (row) => ({ id: row.id, project_id: row.projectId || null, lead_id: row.leadId || null, type: row.type, name: row.name, external_url: row.storagePath ? null : row.url, storage_path: row.storagePath || null, uploaded_by: row.uploadedBy }),
  ...
  leadQuotes: (row) => ({ id: row.id, lead_id: row.leadId, document_id: row.documentId || null, reference: row.reference || '', amount_pence: row.amount == null ? null : pence(row.amount), status: row.status || 'Preparing', sent_at: row.sentAt || null, notes: row.notes || '', project_title: row.projectTitle || '', project_type: row.projectType || '', description: row.description || '', scope: row.scope || [], address: row.address || '', postcode: (row.postcode || '').toUpperCase(), start_date: row.startDate || null, end_date: row.endDate || null, created_by: row.createdBy || null, updated_at: row.updatedAt || new Date().toISOString() }),
}

const tableNames = { clients: 'clients', projects: 'projects', events: 'project_events', tasks: 'tasks', payments: 'payments', expenses: 'expenses', documents: 'documents', photos: 'project_photos', journalEntries: 'journal_entries', activities: 'activity_logs', leads: 'leads', leadCommunications: 'lead_communications', leadQuotes: 'lead_quotes' }
...
export const supabaseRepository = {
  ...
  async load() {
    ...
    const [profiles, clients, projects, events, tasks, payments, expenses, documents, photos, journalEntries, activities, leads, leadCommunications, leadQuotes] = await Promise.all([
      selectAll(client, 'profiles'), selectAll(client, 'clients'), selectAll(client, 'projects'), selectAll(client, 'project_events'), selectAll(client, 'tasks'), selectAll(client, 'payments'), selectAll(client, 'expenses', 'expense_date'), selectAll(client, 'documents'), selectAll(client, 'project_photos'), selectAll(client, 'journal_entries'), selectAll(client, 'activity_logs'), selectAll(client, 'leads'), selectAll(client, 'lead_communications'), selectAll(client, 'lead_quotes'),
    ])
    const data = { version: 6, users: profiles.map(maps.users), clients: clients.map(maps.clients), projects: projects.map(maps.projects), events: events.map(maps.events), tasks: tasks.map(maps.tasks), payments: payments.map(maps.payments), expenses: expenses.map(maps.expenses), documents: documents.map(maps.documents), photos: photos.map(maps.photos), journalEntries: journalEntries.map(maps.journalEntries), activities: activities.map(maps.activities), leads: leads.map(maps.leads), leadCommunications: leadCommunications.map(maps.leadCommunications), leadQuotes: leadQuotes.map(maps.leadQuotes) }
    await Promise.all([...data.documents, ...data.photos, ...data.expenses].map(async (item) => { if (item.storagePath) item.url = await signedUrl(client, item.storagePath) }))
    snapshot = clone(data)
    return data
  },
  async save(data) {
    const client = requireSupabase(); const previous = snapshot || { clients: [], projects: [], events: [], tasks: [], payments: [], expenses: [], documents: [], photos: [], journalEntries: [], activities: [], leads: [], leadCommunications: [], leadQuotes: [] }
    // Documents must exist before quotes reference them, and quotes before leads select them.
    for (const key of ['clients', 'projects', 'documents', 'leadQuotes', 'leads', 'leadCommunications', 'events', 'tasks', 'payments', 'expenses', 'photos', 'journalEntries', 'activities']) await syncCollection(client, key, data[key] || [], previous[key] || [])
    snapshot = clone(data)
    return data
  },
