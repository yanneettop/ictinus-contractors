...
export const leadPatchSchema = z.object({
  clientName: shortText.optional(), email: z.string().email().max(240).optional(), phone: z.string().max(60).optional(), postcode: z.string().max(16).optional(), fullAddress: z.string().max(500).optional(), projectType: shortText.optional(), enquirySummary: optionalText(), estimatedValue: money.optional(), budget: money.optional(), stage: leadStage.optional(), priority: leadPriority.optional(), source: shortText.optional(), sourceReference: z.string().max(240).optional(), assignedTo: z.string().uuid().nullable().optional(), preferredContactMethod: z.enum(['Phone','Email','SMS','WhatsApp']).optional(), preferredContactTime: z.string().max(120).optional(), nextAction: z.string().max(500).optional(), nextActionDueAt: dateTime.nullable().optional(), internalNotes: optionalText(), barkCreditsSpent: z.number().nonnegative().optional(), confirmed: z.boolean().optional(),
}).strict().refine((value) => Object.keys(value).some((key) => key !== 'confirmed'), { message: 'Provide at least one lead field to update.' })
