...
export const integrationOpenApi = {
  ...
  paths: {
    ...
    '/projects/{projectId}/events': { post: { operationId: 'createProjectEvent', summary: 'Create a project event', description: 'Writes to the app database first, then attempts Google Calendar sync. calendarSync is synced, failed, or not_configured; a sync failure never rolls back the database event.', parameters: [projectParameter], requestBody: jsonBody({ $ref: '#/components/schemas/EventCreate' }), responses: { 201: successResponse(), ...standardResponses } } },
    ...
    '/dashboard': { get: { operationId: 'getTodayDashboard', summary: 'Get the aggregated Today Dashboard, urgent priorities and seven-day outlook', responses: { 200: successResponse(), ...standardResponses } } },
